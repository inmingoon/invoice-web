...
- [x] 모바일 360px 가독성 + 다크모드 토글 — **PASS** (2026-05-20 localhost dev, Playwright 360px). `/admin/invoices` 페이지 가로 스크롤 0 / 다크·라이트 모두 가독성 정상. `InvoiceTable`은 shadcn `Table`의 `data-slot="table-container"`(`overflow-x-auto`)를 이미 상속 — 7컬럼(842px)이 328px 컨테이너 내부 스크롤로 격리되어 `overflow-x-auto` 추가 작업 불필요. 증거: `docs/audit/2026-05-20-v7-responsive/v7-admin-mobile-{light,dark}.png`
...
- [x] V7 다크·모바일 360px — **PASS** (2026-05-20 localhost dev, Playwright 360px). 뷰어 `/invoice/[id]` + 관리자 `/admin/invoices` 양쪽 모두 페이지 가로 스크롤 0, 다크모드 대비 정상(`bg lab(2.75)` / `fg lab(98)`). 증거 4장: `docs/audit/2026-05-20-v7-responsive/`. 발견: 뷰어 항목 테이블 금액 컬럼이 360px에서 클리핑 — 페이지 레벨 기준은 충족, UX 폴리시는 v2.1로 이관(아래 §출시 후 확장 후보)
...
- **뷰어 항목 테이블 모바일 레이아웃**: 360px에서 `InvoiceItemsTable`(4컬럼 397px)의 금액 컬럼이 컨테이너 밖으로 클리핑돼 스와이프해야 보임 — 모바일 카드형 레이아웃 또는 컬럼 우선순위 재배치. 2026-05-20 V7 검증 발견. 원화 표기 폭 때문에 단순 패딩 축소로는 부족, 별도 디자인 작업
