...
- [ ] 정상 비밀번호 → `/admin` redirect + 쿠키 발급 — 운영 환경에 `SESSION_SECRET`·`ADMIN_PASSWORD_HASH` 셋업 후 실제 로그인 1회
...
- [ ] V9 복사 풀체인 — dev/Vercel 로그인 → `/admin/invoices` → "링크 복사" 클릭 → 토스트 가시 + 클립보드 값 확인 (1회). secure context(HTTPS·localhost) 필수
...
- [ ] admin 활성화 — `ENABLE_ADMIN=1` + `SESSION_SECRET` + `ADMIN_PASSWORD_HASH` Production env 주입 후 V8(401→로그인→/admin/invoices 200) + V9(복사 풀체인) 라이브 검증 (별도 spec)
